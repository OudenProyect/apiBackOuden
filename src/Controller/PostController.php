...
class PostController extends AbstractController
{
    ...
    #[Route('/postedit', name: 'app_edit', methods: 'POST')]
    public function update(Request $request, HouseRepository $houserepo, CompanyRepository $company, FeatureRepository $feact, LocationServicesRepository $service, PublicationRepository $pos, ImageRepository $imgrepo): JsonResponse
    {
        $datos = [];
        try {
            // IMAGEN
            ...
            // buscar y actualizar publicacion existente
            $publicacionId = $request->get('empresa');
            $publicacion = $pos->find($publicacionId);
            if (!$publicacion) {
                throw new Exception('Error finding publication', 400);
            }
